  ...
  getConstantFunctions(structs) {
    ...
    return this.getConractStatement().body ? Object.assign({},
      ...
      this.createBlockObjects(this.getConractStatement().body.filter(
        elt => elt.type == 'StateVariableDeclaration' && elt.visibility == 'public'),
...
//Work with annotations
  ...
  findAnnotation(annotationEndPos) {
    ...
    var annonations = [];
    while (!str || str.substring(0, 2) == '*/' || str.substring(0, 1) == '*' || str.substring(0, 3) == '/**' || str.substring(0,3) == '///') {

      var lineStr = str.replace(/\/?\*\/?/g, '').replace (/\/\/\//, '').trim();
